...
class TagRepositoryTests: XCTestCase {
    ...
    var repository = FluentMysqlTagRepository()
    
    // MARK: - Overrides
    
    override func setUp() {
        app = try! TestSetup.getApp()
        connection = try! app.requestPooledConnection(to: .mysql).wait()
    }
    ...
    func testGettingTagsForPost() throws {
        ...
        XCTAssertTrue(postTags.contains { $0.name == tag1.name})
        XCTAssertTrue(postTags.contains { $0.name == tag2.name})
    }
    ...
    func testGettingAllTagsWithPostCount() throws {
        ...
        XCTAssertEqual(tagsWithPostCount.first?.0.name, tag2.name)
